...
@Component({
  selector: 'mtb009-outbound',
  templateUrl: './mtb009-outbound.component.html',
  styleUrls: ['./mtb009-outbound.component.scss'],
})

export class Mtb009OutboundComponent implements OnInit, OnDestroy {
  ...
  // Xử lý bộ lọc chính, luôn gọi API lấy danh sách
  private handleFilter(filter: FilterDescriptor[] = null, type: 'text' | 'status' | null = null, resetPage = true) {
    ...
    // Sử dụng setTimeout để tránh double call
    this.filterTimeout = setTimeout(() => {
      this.executeFilter(filter, type, resetPage);
    }, 100);
  }
  ...
  private executeFilter(filter: FilterDescriptor[] = null, type: 'text' | 'status' | null = null, resetPage = true) {
    ...
    // Thêm filter cửa hàng nếu có (đã bỏ filter này)
    // if (!PSObject.isNullOfUndefined(this.headactive.Code) && this.headactive.Code != null) {
    //   this.groupfilter.filters.push({
    //     field: 'Code',
    //     operator: 'eq',
    //     value: this.headactive.Code
    //   });
    // }
    ...
    if (this.filterstatus.filters.length != 0 || this.filtertext.filters.length != 0) {
      ...
      if (this.filterstatus.filters.length != 0) {
        ...
        let listmap = [WHIOMasterStatusEnum.NEW, WHIOMasterStatusEnum.PENDING];
        this.isDisabledReset = PSArray.areEqual(listfilter, listmap) && this.filtertext.filters.length == 0;
      }
  ...
  public onIOMasterTypeSelect(id: number | null): void {
    ...
    this.isLoadingData = true; // Bắt đầu loading
    this.getliststatus(); // Sau khi lấy trạng thái xong sẽ gọi handleFilter
  }

  // Thay đổi bộ lọc text
  public textFilterChange(e: FilterDescriptor[]) {
    if (this.isLoadingData) return; // Không cho phép filter khi đang loading
  ...
  // Lấy danh sách cửa hàng, sau đó lấy trạng thái, cuối cùng mới filter
  private getlisthead() {
    this.subLoader.loader(true);
    this.isLoadingData = true; // Bắt đầu loading
    var temp = this.coreapi.GetListHead().subscribe((res) => {
      if (res.StatusCode == 0) {
        this.listhead = [...this.listheadcopy, ...res.ObjectReturn];
        this.headactive = this.listhead[0];
        this.subLoader.loader(false);
        this.getliststatus(); // Chỉ gọi tiếp khi đã có head
      } else {
        this.subLoader.loader(false);
  ...
  private getliststatus() {
    ...
    var temp = this.coreapi.GetListStatus(LSStatusTypeDataEnum.IO).subscribe((res) => {
      if (res.StatusCode == 0) {
        ...
        this.subLoader.loader(false);
        this.isLoadingData = false;
        this.handleFilter(); // Chỉ gọi sau khi đã có trạng thái
      } else {
        this.subLoader.loader(false);
        this.isLoadingData = false;
