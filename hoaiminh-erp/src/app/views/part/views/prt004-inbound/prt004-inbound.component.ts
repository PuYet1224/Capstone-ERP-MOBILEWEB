...
import { WHIOMasterTypeOfMasterEnum, WHIOMasterTypeOfMasterConfig } from '../../../../models/enums/e-type/wh-io-master-type-of-master.enum';
...
@Component({
  selector: 'prt004-inbound',
  templateUrl: './prt004-inbound.component.html',
  styleUrls: ['./prt004-inbound.component.scss']
})
export class Prt004InboundComponent implements OnDestroy, OnInit {
  ...
  public inboundTypeList = Object.values(WHIOMasterTypeOfMasterConfig)
    .filter(k => k.id != WHIOMasterTypeOfMasterEnum.AfterStock)
  ...
  public createInbound(item: { id: number | null }): void {
    ...
    dto.TypeData = WHIOMasterTypeDataEnum.In
  ...
  private typedataFilter: FilterDescriptor = {
    ...
    operator: 'eq',
    ignoreCase: false,
    value: WHIOMasterTypeDataEnum.In
  };
  ...
  public listfiltertext = [
    'DocumentID', 'RefDocumentID', 'SupplierName',
    'OutHeadName', 'InHeadName', 'Description'
  ];
  public skip = 0;
  ...
  public typeMasterList = [
    ...Object.values(WHIOMasterTypeOfMasterConfig).filter(x => x.active)
  ];
  ...
  public itemAction: WHIOMasterCusDTO | null = null;
  ...
  public onUpdateIOMasterStatus(param: WHIOMasterCusDTO, enumStt: WHIOMasterStatusEnum): void {
    const item: UpdateStatusInterface<WHIOMasterCusDTO> = {
      ListDTO: [param],
      Status: enumStt
    };
    this.UpdateIOMasterStatus(item);
  }

  public onActionClick(e: ActionColumnDTO) {
    this.itemAction = e.data;
    if (e.action == 'view' || e.action == 'edit') {
      this.cache.setItem(KeyLocalStorageEnum.IOMASTER_OBJECT, this.itemAction);
      this.router.navigate(['detail'], { relativeTo: this.route });
      ...
      if (this.itemAction.TotalSKUNotEqual > 0)
      ...
      (res) => {
        if (res.StatusCode == 0) {
          ...
          this.notification.onError(
            `Lỗi lấy danh sách phiếu nhập: ${res.ErrorString}`
          );
        }
      },
      (err) => {
        this.subLoader.loader(false);
        this.notification.onError(`Lỗi lấy danh sách phiếu nhập: ${err.message}`);
      }
    );
    this.arrUnsubscribe.push(temp);
  }

  public UpdateIOMasterStatus(param: UpdateStatusInterface<WHIOMasterCusDTO>) {
    this.subLoader.loader(true);
    ...
    const sub = this.partapi.UpdateIOMasterStatus(param).subscribe(
      res => {
        ...
        if (res.StatusCode === 0) {
          ...
          this.notification.onError(`Lỗi ${txt} không thành công: ${res.ErrorString}`);
        }
      },
      err => {
        this.subLoader.loader(false);
