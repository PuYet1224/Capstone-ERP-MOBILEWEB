...
@Component({
  selector: 'prt005-inbound-detail',
  templateUrl: './prt005-inbound-detail.component.html',
  styleUrls: ['./prt005-inbound-detail.component.scss'],
})
export class Prt005InboundDetailComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.inhead = psConfig.GetHead().Head;
  }
  ...
  ngOnInit(): void {
    ...
    switch (this.iomasterCache.TypeOfMaster) {
      ...
      case WHIOMasterTypeOfMasterEnum.Supplier:
    ...
    this.GetListWarehouse(this.inhead, true);

    var head = this.header.headObs$.subscribe((data) => {
      if (data != null) {
        this.router.navigate(['/part/warehouse/inbound']);
  ...
  public isShowDialogDoneFunc() {
    if (this.notEqualQuantity == 0)
      this.UpdateIOMasterStatus(WHIOMasterStatusEnum.DONE)
    else
  ...
  public listWHIn: LSWarehouseCusDTO[] = [];
  private listSupplierFull: LSTypeOfPartnerCusDTO[] = [];
  ...
  public onOutWHChanged(value: number): void {
    this.iomaster.OutHead = value;
    this.onFieldChanged(this.iomaster, ['OutHead']);
    this.iomaster.OutWH = null;
    if (value == null) {
      this.listWH = [
        {
          Code: null,
  ...
  public onFieldChanged(dto: WHIOMasterCusDTO, prop: string[]) {
    var list: string[];
    if (dto.Code == 0) {
      dto.InHead = this.inhead;
      list = ['TypeData', 'TypeOfMaster', 'InHead'];
      list.push(...prop);
    } else list = prop;

    if (this.iomaster.Code != 0 && this.iomaster[prop[0]] == this.refiomaster[prop[0]])
      return;

    var dtoupdate: UpdatePropertiesInterface<WHIOMasterCusDTO> = {
      DTO: dto,
      Properties: list,
    };
    this.UpdateIOMaster(dtoupdate);
  }

  public supplierValueChange(v: string) {
    var temp = this.listSupplier.find((f) => f.Name == v);
    this.iomaster.Supplier = temp.Code;
    this.onFieldChanged(this.iomaster, ['Supplier']);
  }
    ...
    var temp = this.partapi.GetIOMaster(param).subscribe(
      (res) => {
        if (res.StatusCode == 0) {
          ...
          if (this.iomaster.OutHead && !reload)
            this.GetListWarehouse(this.iomaster.OutHead);
  ...
  private GetListWarehouse(headCode: number, isIn: boolean = false) {
    this.subLoader.loader(true);
    var temp = this.coreapi.GetListWarehouse(headCode).subscribe(
      (res) => {
        if (res.StatusCode == 0) {
          ...
          if (isIn) {
            this.listWHIn = res.ObjectReturn;
            this.listWHIn.unshift(firstItemWH);
          } else {
  ...
  private getListHead() {
    ...
    var temp = this.coreapi.GetListHead().subscribe(
      (res) => {
        if (res.StatusCode == 0) {
          this.listHead = res.ObjectReturn.filter((f) => f.Code != this.inhead);
  ...
  private GetListSupplier() {
    ...
    var temp = this.coreapi.GetListSupplier().subscribe(
      (res) => {
        if (res.StatusCode == 0) {
          this.listSupplierFull = res.ObjectReturn;
          this.listSupplier = [...this.listSupplierFull];
          this.subLoader.loader(false);
        } else {
          this.subLoader.loader(false);
          this.notification.onError(
            `Lỗi lấy thông tin nhà cung cấp: ${res.ErrorString}`
          );
        }
      },
      (err) => {
        this.subLoader.loader(false);
        this.notification.onError(
          `Lỗi lấy thông tin nhà cung cấp: ${err.message}`
        );
      }
    );
    this.arrUnsubscribe.push(temp);
  ...
  public onSupplierFilterChange(keyword: string): void {
    if (!keyword || keyword.trim() === '') {
      this.listSupplier = [...this.listSupplierFull];
    } else {
      const lowerKey = keyword.toLowerCase();
      this.listSupplier = this.listSupplierFull.filter(item =>
