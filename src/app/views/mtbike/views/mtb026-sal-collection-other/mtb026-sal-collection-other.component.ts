...
export class Mtb026SalCollectionOtherComponent {
  ...
  // private getlistcslist(param: CSListTypeDataEnum, forceReload: boolean = false) {
  //   this.subLoader.loader(true);
  //   var temp = this.configCache.GetListCSList(param, forceReload).subscribe((res) => {
  //     if (res) {
  //       if (param == CSListTypeDataEnum.INSTALLAPP) {
  //         this.listHondaReason = res;
  //       } else {
  //         this.listOAReason = res;
  //       }
  //       this.subLoader.loader(false);
  //     } else {
  //       this.subLoader.loader(false);
  //       this.notification.onError(`Lỗi lấy danh sách lý do`);
  //     }
  //   }, (err) => {
  //     this.subLoader.loader(false);
