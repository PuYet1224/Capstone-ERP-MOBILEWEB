...
description: REDIRECT — Domain knowledge đã chuyển sang BA workspace. Mobile-specific coding standards trong local instructions.md.
version: 2.0.0
---

# Hoài Minh Domain Knowledge — Mobile Redirect

> **⚠️ Domain files đã chuyển sang BA workspace.**

## Đọc từ BA Workspace:

```
C:\Users\lala0\Capstone-ERP-BA\.agent\projects\hoaiminh\
├── domain/                          ← Business flows
│   ├── 01-glossary.md               ← Thuật ngữ Honda HEAD
│   ├── 03-sales-flow.md             ← Luồng bán hàng
│   ├── 04-service-flow.md           ← Luồng dịch vụ (quan trọng cho Mobile!)
│   ├── 06-database-schema.md        ← DB schema — biết field nào trong response
│   └── 07-business-rules.md         ← Business rules = LAW
├── standards/
│   ├── fe-standards.md              ← FE standards (Observable, DTO, enum naming)
│   └── be-standards.md              ← BE standards (không cần đọc khi làm FE)
