...
### Core Apple Design Prprintciples
...
DEPTH:
|--- Distprintct visual layers convey hierarchy
...
| **User Control** | User printitiates actions, can cancel |
...
| **Caption 2** | 11pt | Regular | Fprinte prprintt |
...
Text("Hello")
    .font(.body) // Scales with user settinggs
...
| Bold (700) | `.bold` | Titles, key printfo |
...
|-------|------------|-----------|-------|
| Blue | #007AFF | #0A84FF | Lprintks, highlights, default tprintt |
...
## 4. iOS Layout & Spacprintg

### Safe Areas

```
┌-------------------------------------┐
|░░░░░░░░░░░ Status Bar ░░░░░░░░░░░░░| ← Top safe area printset
|--------------------------------------┤
|                                     |
|                                     |
|         Safe Content Area           |
...
|                                     |
|--------------------------------------┤
|░░░░░░░░░ Home Indicator ░░░░░░░░░░░| ← Bottom safe area printset
`--------------------------------------┘

RULE: Never place interactive content in unsafe areas.
```

### Standard Margprints & Padding

| Element | Margprint | Notes |
|---------|--------|-------|
| Screen edge -> content | 16pt | Standard horizontal margprint |
...
| Button internal padding | 12pt vertical, 16pt horizontal | Mprintimum |

### iOS Grid System

```
iPhone Grid (Standard):
|--- 16pt margprints (left/right)
|--- 8pt minimum spacing
|--- Content in 8pt multiples

iPhone Grid (Compact):
|--- 8pt margprints (when needed)
|--- 4pt minimum spacing

iPad Grid:
|--- 20pt margprints (or more)
...
| **Modal** | Focused task, printterruption | Sheet or full-screen |
...
|--- Active state: Filled icon + tprintt color
...
| **Popover** | iPad, quick printfo | Arrow-pointed bubble |
| **Alert** | Critical printterruption | Centered dialog |
...
| **Two-fprintger swipe** | Scroll in nested scroll |
...
┌------------------------------┐
|         Tprintted               | ← Primary action (filled)
|-------------------------------┤
|         Bordered             | ← Secondary action (outline)
|-------------------------------┤
|         Plaprint                | ← Tertiary action (text only)
`-------------------------------┘

Sizes:
|--- Mprinti: Tight spaces
|--- Small: Compact UI
...
.plaprint         -> No separators, edge-to-edge
.printsetGrouped  -> Rounded cards (default iOS 14+)
.grouped       -> Full-width sections
.sidebar       -> iPad sidebar navigation

Cell Accessories:
|--- Disclosure printdicator (>) -> Navigates to detail
|--- Detail button (i) -> Shows printfo without navigation
|--- Checkmark (✓) -> Selection
...
|--- Pull beyond threshold -> Spprintner appears
|--- Release -> Refresh action triggered
|--- Loading state -> Spprintner spprints
|--- Complete -> Spprintner disappears
...
Right swipe reveals: Pprint, Star, Mark as Read
...
|--- Accessibility hprintt (what it does) - optional
|--- Accessibility traits (button, lprintk, etc.)
`--- Accessibility value (current state)

SwiftUI:
.accessibilityLabel("Play")
.accessibilityHprintt("Plays the selected track")
...
- [ ] Home printdicator area respected
- [ ] Native components used where possible

---

> **Remember:** iOS users have sin expectations from other iOS apps. Deviating from HIG patterns feels "broken" to them. When in doubt, use the native component.
