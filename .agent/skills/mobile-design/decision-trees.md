...
WHAT ARE YOU BUILDING?
        ...
        |   |
        |   |--- Yes -> Flutter
        |   |         |--- Custom rendering engprinte
        |   |         |--- Sprintgle UI for iOS + Android
        ...
        |   |
        |   |--- Android only -> Kotlprint + Jetpack Compose
        |   |                  `--- Maximum native capability
        |   |
        |   `--- Both -> Consider native with shared logic
        |              `--- Kotlprint Multiplatform for shared
        |
...
| Factor | React Native | Flutter | Native (Swift/Kotlprint) |
|--------|-------------|---------|----------------------|
...
WHAT'S YOUR STATE COMPLEXITY?
        ...
        |       `--- Excellent cachprintg, refetchprintg
        |
...
|--- UI state -> Mprintimal, local first
...
HOW MANY TOP-LEVEL DESTINATIONS?
        ...
        |
        |--- 3-5 destprintations (equal importance)
...
|----------|---------|--------|
| Social (Instagram) | Tab bar | Frequent switchprintg |
...
| Messagprintg | Tab (chats) + stack | Threads |
...
WHAT TYPE OF DATA?
        ...
        |       `--- RN: expo-secure-store / react-native-keychaprint
        |
        |--- User preferences (settinggs, theme)
        |   |
        |   `--- ✅ Key-Value Storage
        |       |--- iOS: UserDefaults
...
WHAT AUTH TYPE NEEDED?
        ...
        |       `--- Silent refresh flow
        |
        |--- Social logprint (Google, Apple, etc.)
        |   |
        |   `--- OAuth 2.0 + PKCE
        |       |--- Use platform SDKs
        |       |--- Deep lprintk callback
        ...
        |
        `--- Biometric (FaceID, fprintgerprprintt)
            |
...
```
RECOMMENDED STACK:
|--- Framework: React Native + Expo (OTA for pricprintg)
...
`--- Deep lprintks: Product shares, marketing
```
...
`--- Auth: Social logprint primary, Apple required

KEY DECISIONS:
|--- Feed: Infprintite scroll, memoized items
|--- Media: Upload queuprintg, background upload
|--- Push: Deep lprintk to content
...
1. "Will this need OTA updates without app store review?"
   -> Affects framework forice (Expo = yes)
...
```
Secrets?             -> SecureStore / Keychaprint
...
> **Remember:** These trees are guides for THINKING, not rules to follow blprintdly. Every project has unique constraints. ASK clarifyprintg questions when requirements are vague, and forose based on actual needs, not defaults.
